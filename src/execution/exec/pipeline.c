...
{
	return (node && node->type == G_PIPELINE
		&& (node->sibling == NULL || node->sibling->type != G_PIPELINE));
}

static void	execute_first_pipeline(t_ast_node *pipeline,
		t_data *data, int pipefd[2])
{
	pid_t	pid;
...
static void	execute_middle_pipeline(t_ast_node *pipeline,
		t_data *data, int pipefd[2])
{
	pid_t	pid;
	int		saved_stdin;
...
static pid_t	execute_last_pipeline(t_ast_node *pipeline,
		t_data *data, int pipefd[2])
{
	pid_t	pid;
